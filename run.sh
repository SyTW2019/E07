...
case "$1" in
  start)
    ...
    pm2 start ${APP_FOLDER}/server.js --name node-todo --uid usuario g --gid bitnami
    exit $?
    ;;
  stop)
    pm2 stop node-todo --uid usuario g --gid bitnami
    exit $?
    ;;
  restart|force-reload|reload)
    pm2 restart node-todo --uid usuario g --gid bitnami
    exit $?
    ;;
  init)
    ...
    if [[ ! -f ${DATA_FOLDER}/.initialized ]]; then
      ...
      # Fix permissions
      # We set usuario g as user and group as is the user that will run the service.
      chown -R usuario g usuario g ${DATA_FOLDER}
    ...
    # Fix permissions
    # We set usuario g as user and group as is the user that will run the service.
    chown -R usuario g usuario g ${APP_FOLDER}
